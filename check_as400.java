...
/*
		while(buffer1.indexOf("READY")!=-1){
			 buffer1 = buffer1.substring(buffer.indexOf("READY") + 0); 
       count1++;
		}*/
		BufferedReader reader = new BufferedReader(new StringReader(buffer1));
		try {
			while ((temp = reader.readLine()) != null )
			{
				...
				{
					temp2 = temp;
					while((temp2.indexOf("READY")) != -1)
					{
						temp2 = temp2.substring(temp2.indexOf("READY") + 8 );
						count1++;
					}//Fin While READY
                                }//Fin if Ready
			}//Fin While principal READY.
                     } catch (IOException e){ e.printStackTrace(); }   
                
                BufferedReader reader1 = new BufferedReader(new StringReader(buffer1));  
                try{
                        while((temp = reader1.readLine()) != null){
                        if (temp.contains(""))
				{
					temp2= temp;
					while((temp2.indexOf("OPENED"))!= -1)
					{
					temp2=temp2.substring(temp2.indexOf("OPENED") + 8);
					count++;	
						}//Fin While CLOSED
					}// Fin If Closed
                        }//Fin While principal CLOSED
		} catch (IOException e){ e.printStackTrace(); }
...
//			 buffer = buffer.substring(buffer.indexOf("OPENED") + 6); 
  //     count++; 
	...
	//}
		...
		System.out.println("There are "+count+" OPENED / "+count1+" READY status problems. | prb="+count+";1;1;0; ");
		return returnStatus;
	}	
